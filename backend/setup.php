...
try {
    // Create unified MVC schema tables
    ...
    if (file_exists($legacy_file)) {
        $legacy = new PDO("sqlite:$legacy_file");
        $legacy->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $oldUsers = $legacy->query("SELECT * FROM users")->fetchAll(PDO::FETCH_ASSOC);
        foreach ($oldUsers as $old) {
            $check = $pdo->prepare("SELECT id FROM users WHERE email = ?");
            $check->execute([$old['email']]);
            if (!$check->fetch()) {
                $ins = $pdo->prepare("INSERT OR IGNORE INTO users (full_name, index_number, email, phone, password_hash, role) VALUES (?, ?, ?, ?, ?, ?)");
                $ins->execute([
                    $old['name'] ?? 'User',
                    $old['email'],
                    $old['email'],
